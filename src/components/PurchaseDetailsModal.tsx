...
import { X, Mail, Phone as PhoneIcon, Monitor } from 'lucide-react';
import { Translations } from '../translations/en';

interface PurchaseDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (details: { email: string; phone: string; anydesk: string; }) => void;
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in-up">
      <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 w-full max-w-md shadow-2xl shadow-purple-500/10">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">{translations.anydeskLabel}</label>
            <div className="relative">
              <Monitor className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
              <input type="text" value={customerAnydesk} onChange={(e) => setCustomerAnydesk(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-slate-700 border border-slate-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:border-cyan-500" placeholder="123 456 789" />
            </div>
          </div>
